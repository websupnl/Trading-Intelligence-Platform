...
@celery_app.task(name="app.tasks.signal_tasks.generate_scalp_signals")
def generate_scalp_signals():
    """Intraday scalp signals op 15min/1H charts. Draait elke 5 minuten, crypto 24/7."""
    from app.services.signal_generator import SignalGeneratorService
    from app.services.ai_guard import is_ai_paused
    try:
        if is_ai_paused():
            return {"status": "skipped", "reason": "ai_paused"}
        svc = SignalGeneratorService()
        count = asyncio.run(svc.generate_scalp_signals())
        logger.info(f"Scalp signalen: {count} gegenereerd")
        if count > 0:
            celery_app.send_task("app.tasks.analysis_tasks.auto_trade")
